...
try:
    ...
    AppConfig = smart_import('config', 'AppConfig')
    create_app_config = smart_import('config', 'create_app_config')
    create_petre_orchestrator = smart_import('main', 'create_petre_orchestrator')
    run_petre_from_config = smart_import('main', 'run_petre_from_config')
    quick_evaluation = smart_import('main', 'quick_evaluation')
    PETREError = smart_import('interfaces', 'PETREError')
    ValidationError = smart_import('interfaces', 'ValidationError')
...
def main():
    ...
    try:
        ...
        print(f"PETRE evaluation completed successfully")
...
def run_from_config(config_path: str, **overrides) -> Dict[str, Any]:
    ...
    if overrides:
        return quick_evaluation(config_path, **overrides)
...
class PETRE():
    ...
    def incremental_execution(self, verbose:bool=True):
        ...
        # Get and store initial annotations
        annotations = self.dataset.get_annotations()
        annotations_filepath = os.path.join(self.output_folder_path, "Annotations_PETRE_Start.json")
        ...
        ranks_file_path = os.path.join(self.output_folder_path, f'Ranks_Start.csv')
    ...
    def petre(self, k:int, plot_explanations:bool=False, verbose:bool=True):
        ...
        total_n_steps = 0
        annotations_file_path = os.path.join(self.output_folder_path, f'Annotations_PETRE_k={k}.json')
        ...
        for idx, (text, label) in tqdm(enumerate(zip(texts_column, labels_idxs)), total=len(texts_column),
            ...
            for sentence in doc.sents:
                ...
                if sent_token_count > self.tokenizer.model_max_length:                    
                    error_text = f"ERROR: Sentence with length {sent_token_count} > {self.tokenizer.model_max_length} at index {idx} with label {label}: {sentence_txt}"
